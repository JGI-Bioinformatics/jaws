...
#    def get_tasks(self):
#        """
#        Get all cromwell_job_ids associated with this run.
#
#        :return: list of task_ids
#        :rtype: list
#        """
#        # Note: jaws' "task_id" is identical to cromwell's "job_id"
#        # Whereas jaws' "job_id" refers to cluster (e.g. slurm) reservation.
#        logger.info(f"Get tasks for Run {self.run_id}")
#        if self.data.cromwell_run_id is None:
#            return []
#        try:
#            metadata = cromwell.get_metadata(self.data.cromwell_run_id)
#            task_ids = metadata.get_job_ids()
#        except Exception as error:
...
#        return task_ids
    ...
    def __check_if_running(self):
        """
        Query the jaws-task service to see if any tasks are running; this is required to
        move the Run from "queued" to "running" state.
        """
        logger.info(f"Check if Run {self.run_id} started running")

        # get task ids from Cromwell metadata
        task_ids = self.get_tasks()

        # get task status from jaws-task service
        task_service = api.task_service.TaskService()
        try:
            response = task_service.get_status({"task_ids"=check_task_ids.keys()})
        except Exception:
            raise

        # determine when earliest task started
        started = None
        for task_id in response["result"]:
            if response["result"][task_id]["status"] == "running":
                timestamp = response["result"][task_id]["timestamp"]
                if not started or timestamp < started:
                    started = timestamp

        # if started, update state
        if started:
            self.__update_status("running", timestamp=started)
    ...
    def __update_status__(self, new_status, timestamp=datetime.utcnow(), reason=""):
        """
