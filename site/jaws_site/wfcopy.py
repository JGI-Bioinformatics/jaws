...
Input cromwell directory name must end with the job id (e.g, cromwell-executions/fungal_min/4a14ec42-2d1c-4b65-9c4f-9f3a6307a094)
...
def getArgs():
    ...
    '''
    parser = argparse.ArgumentParser(description=progDesc, formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('-fs', '--flattenShardDir', dest='flattenShardDir', action='store_true', help='copy contents of shard dirs into one dir.')
    parser.add_argument('-v', '--verbose', dest='verbose', action='store_true', help='print messages to stdout.')
...
def wfcopy(cromwellDir, outDir, flattenShardDir=False, verbose=False):
    ...
    for rootDir, subdirs, files in os.walk(cromwellDir):
        ...
        if os.path.basename(parentDir).startswith('shard-'):
            ...
            if subwfname: shardname = "%s-%s"%(subwfname, shardname)
        ...
        if rootDir.endswith('execution'):
            ...
            if not os.path.exists(taskDir): os.makedirs(taskDir)
            ...
            for fname in files:
                ...
                if fname == 'stdout': rsync(fullname, os.path.join(logdir, "%s.stdout"%outname), verbose=verbose)
                if fname == 'stderr': rsync(fullname, os.path.join(logdir, "%s.stderr"%outname), verbose=verbose)
                if fname == 'script': rsync(fullname, os.path.join(logdir, "%s.script"%outname), verbose=verbose)
                if fname not in cromwellFilesToSkip: rsync(fullname, taskDir, verbose=verbose)
                if fname == 'rc':
...
if __name__ == '__main__':
    ...
    wfcopy(cromwellDir, outDir, flattenShardDir=argParser.flattenShardDir, verbose=argParser.verbose)
    sys.exit()
