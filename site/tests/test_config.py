...
def test_overwrite_all_default_values(config_file):
    ...
    expected_amqp_sections = [("host", "currenthost"),
                              ("vhost", "current_vhost"),
                              ("user", "daffy_duck"),
                              ("password", "succotash"),
                              ("queue", "rabbit_season")]

    expected_rpc_sections = [("host", "currenthost"),
                             ("vhost", "current_vhost"),
                             ("user", "bugs_bunny"),
                             ("password", "doc"),
                             ("queue", "duck_season"),
    ...
    expected_globus_sections = [("client_id", "foghorn_leghorn"),
                                ("endpoint_id", "rooster"),
                                ("root_dir", "cwd"),
                                ("subdir", "cwd")]

    expected_db_sections = [("host", "myhost"),
                            ("port", "60032"),
                            ("user", "elmer_fudd"),
                            ("password", "hunting"),
                            ("db", "hunting_sites")]

    expected_cromwell_sections = [("host", "current_host"),
                                  ("port", "8000")]
...
def test_config_overwrite_partial_values(partial_config):
    ...
    check_section("RPC", [("host", "localhost")], cfg)
