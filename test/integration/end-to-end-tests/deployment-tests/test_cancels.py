...
FINAL_STATE = "done"
VALID_STATES = ["running", "queued", "submitted"]
CHECK_TRIES = 20000
CHECK_SLEEP = 5


def test_cancel(dir, site):
    wdl = dir + WDL
    input_json = dir + INP
    ...
    floating_state = ""
    tries = 0
    while floating_state not in VALID_STATES:
        r_sts, o_sts, e_sts = util.run(status_cmd)
        floating_state = json.loads(o_sts)["status"]

        if floating_state == FINAL_STATE:
            error_message = "One of the following states were not seem: %s" % VALID_STATES
