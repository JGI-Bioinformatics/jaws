...
from jaws_jtm.lib.config import COMPUTE_RESOURCES, CORI_CHARGE_ACCNT, \
        CORI_CONSTRAINT, CORI_QOS, MEMPERNODE, NNODES, NWORKERS, QOS_LIST
...
def submit():
    ...
    parser.add_argument(
        "-cl", "--cluster", dest="cluster_name", choices=COMPUTE_RESOURCES
    )
    group.add_argument(
        "-cr", "--cromwell", dest="cromwell"
    )  # generic for command submission ex) jtm-submit -cr "ls"
    group.add_argument("-f", "--task-json-file", dest="taskFile")
    group.add_argument("-j", "--task-json-str", dest="task_in_json_str")
    parser.add_argument(
        "-l",
        "--loglevel",
    ...
    parser.add_argument("-A", "--account", dest="account", default=CORI_CHARGE_ACCNT)
    parser.add_argument(
        "-c", "--cpu", help="Number of cores", type=int, dest="num_cpus"
    )
    parser.add_argument(
        "-C",
        "--constraint",  # only for nersc
    ...
    parser.add_argument("-p", "--pool-name", dest="pool_name", default="small")
    parser.add_argument("-q", "--qos", dest="qos", choices=QOS_LIST, default=CORI_QOS)
    parser.add_argument(
        "-s",
        "--shared",
        help="Shared workers.",
    ...
    parser.add_argument("-t", "--time", dest="job_time")
    ...
    if args.job_time:
        assert (
            args.num_cpus and args.memory and args.num_nodes and args.pool_name
    ...
    ret = int(
        JtmInterface("task", info_tag=args.pool_name if args.pool_name else None).call(
            ...
            # pool_size=args.pool_size,  # not used
