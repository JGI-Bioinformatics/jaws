...
JTM_SQL = {
    ...
    "select_workerid_workers_by_lifeleft_jtmhostname": """
        SELECT workerId
        FROM workers
        WHERE lifeLeft >= 0 and jtmHostName = '%(jtm_host_name)s'""",
    ...
    "update_workers_lifeleft_by_workerid": """
        UPDATE workers
        SET lifeLeft = -1
        WHERE workerId = '%(worker_id)s' and jtmHostName = '%(jtm_host_name)s'""",
    ...
    "update_runs_status_workerid2_by_taskid_2": """
        UPDATE runs
        SET status = %(status_id)d, workerId2 = %(wid2)d, endDate = '%(now)s'
        WHERE taskId = %(task_id)d;""",
    ...
    "update_runs_tid_startdate_by_tid": """
        ...
        WHERE taskId = %(task_id)d and status < 1;""",
