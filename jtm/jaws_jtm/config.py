...
# This is a test
# Todo: Set a rest api to recv the key
#  This key should be distributed after authentication by REST
# Note: How to gen
# import base64
# >>> from cryptography.fernet import Fernet
# >>> key = b'pRmgMa8T0INjEAfksaq2aafzoZXEuwKI7wDe4c1F8AY='
# >>> cipher_suite = Fernet(key)
# >>> ciphered_text = cipher_suite.encrypt(b"xxxxxxxxxxxxx")
# >>> print(ciphered_text)
#
...
if JTM_HOST_NAME == "cori":
    if PRODUCTION:
        if LOGIN_NAME == "jaws" and JTM_HOST_NAME == "cori":
            ...
            ENV_ACTIVATION = "source /global/cfs/projectdirs/jaws_jtm/anaconda3/bin/activate " \
                             "/global/cfs/projectdirs/jaws_jtm/dev/jtm"
    else:
        if LOGIN_NAME == "jaws" and JTM_HOST_NAME == "cori":
            # For jaws on cori20
            ENV_ACTIVATION = "source /global/cfs/projectdirs/jaws/jtm/venv-dev/bin/activate"
        else:
            ENV_ACTIVATION = "source /global/cfs/projectdirs/jaws_jtm/anaconda3/bin/activate " \
                             "/global/cfs/projectdirs/jaws_jtm/prod/jtm"
elif JTM_HOST_NAME == "jaws_lbl_gov":
    if PRODUCTION:
        ENV_ACTIVATION = "source /global/home/groups-sw/lr_jgicloud/anaconda3/bin/activate " \
                         "/global/home/groups-sw/lr_jgicloud/prod/jtm"
    else:
        ENV_ACTIVATION = "source /global/home/groups-sw/lr_jgicloud/anaconda3/bin/activate " \
                         "/global/home/groups-sw/lr_jgicloud/dev/jtm"
...
if JTM_HOST_NAME == "cori":
    ...
    if PRODUCTION:
        ...
        JTM_LOG = os.path.join(os.environ["SCRATCH"], "dev/jtm/%s/logs" % (CNAME))
...
if JTM_HOST_NAME in ("cori"):
    ...
    RMQ_USER = CIPHER_SUITE.decrypt("gAAAAABdrhu9Cu4sWDP7625IsLLLZgKy3mF_5X-_18QQGzSn4H8D5cix5C5oi4m2EXzvJtL0t6JFemooi1\
xmB_gLIv88JXshtg==".encode())
    RMQ_PASS = CIPHER_SUITE.decrypt("gAAAAABdrhgBIKwNC0DRX6zZMitFesyOjWgeFuuhHVeTHtWiNfKUhuo6YBIje8l2abqUKCpmtXCbCSKIS0\
u9J3AgHRu5TPAPvG3rdw4s1V33OsIeEdEfshU=".encode())
    RMQ_VHOST = "jgi"
    if JTM_HOST_NAME in ("cori"):
        RMQ_PORT = "5672"
    else:
    ...
    RMQ_USER = CIPHER_SUITE.decrypt("gAAAAABd1ssdmSoy6MtbPocGnbJoD82kXYXI2WkX8hAt976bOe4YBQsz8yQJCiwdQG84TgdmMh2J6i0y5-\
3j2-KHpcuNelohsQ==".encode())
    RMQ_PASS = CIPHER_SUITE.decrypt("gAAAAABd1ssdmSoy6MtbPocGnbJoD82kXYXI2WkX8hAt976bOe4YBQsz8yQJCiwdQG84TgdmMh2J6i0y5-\
3j2-KHpcuNelohsQ==".encode())
    RMQ_VHOST = "jaws"
    RMQ_PORT = "5672"
elif JTM_HOST_NAME in ("summit", "rhea", "slate"):
...
JGI_JTM_MAIN_EXCH = "jgi_jtm_main_exch_" + CNAME   # exchange for user task request and replay b/w JWS and JTM
if not PRODUCTION:
...
# In Worker
WORKER_HB_RECV_INTERVAL = 6.0   # client->worker heartbeat receiving interval in worker NOT USED!!
...
JTM_WORKER_NUM_THREADS = 5   # used in resourceusage.py for getting the number of workers by ps
...
if JTM_HOST_NAME in ("cori"):
    ...
    if PRODUCTION:
        ...
        MYSQL_PORT = 60006
        MYSQL_PW = CIPHER_SUITE.decrypt("gAAAAABdrhip0dSjHxtw6KVgIKTOFWrdAS-TzTqv60WvYN7bMx\
    ls5cHaFviIEwfeQByeAsHKQVv6U1oz4gccuwRPfV983rsSAJsuU5YKFIxqJ5XbtDu30so=".encode())
    else:
        MYSQL_HOST = "db.mysql.dev-cattle.stable.spin.nersc.org"
        MYSQL_PORT = 60005
        MYSQL_PW = CIPHER_SUITE.decrypt("gAAAAABdrhoByzbVJVCTH3gV7Wb8aqowrywBx3mccnKvb7nWO-\
    HsMq1nBdteaXW816UIjZBzkWze7ss1vuVEqjVVEAHqsfRJBR_Jp9L1wE9PxZurVNH-1F0=".encode())
elif JTM_HOST_NAME in ("lawrencium", "jgi_cloud", "jaws_lbl_gov"):
    if PRODUCTION:
        MYSQL_USER = "jtm"
        MYSQL_DB = "jtm"
...
# memory pool per core (should be use with -c, ie. core based request)
...
# cloning time rate = life_left / runtime_requested ex) if -t=5min, fire sbatch at 4min
CTR = 0.2
