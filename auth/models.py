...
class User(Base):
    ...
    def update(self, name, email, auth_access_token, auth_refresh_token, auth_expires_at_seconds, transfer_access_token, transfer_refresh_token, transfer_expires_at_seconds, groups_access_token, groups_refresh_token, groups_expires_at_seconds):
        self.name = name
        self.email = email
    ...
    #def dump(self): return dict([(k, v) for k, v in vars(self).items() if not k.startswith('_')])
