...
class PoolManagerPandas:
    ...
    def __init__(
        self, condor_provider=None, slurm_provider=None, configs=None, **kwargs
    ):
        self.site = "nersc"
    ...
    def determine_condor_job_sizes(self, condor_jobs: Dict) -> Dict:
        ...
        mask_over = df["cpu_bin"].str.contains("over") | df["mem_bin"].str.contains(
            "over"
        )
        ...
        for compute_type in self.configs["compute_types"]:
            ...
            condor_q_status[f"idle_{compute_type}"] = sum(mask_type & mask_idle_status)
    ...
    def need_new_nodes(
        self, condor_job_queue: Dict, slurm_workers: Dict, machine_size: str
    ) -> Dict:
        """
...
if __name__ == "__main__":
    ...
    pool = PoolManagerPandas(
        condor_provider=HTCondor(columns=wanted_columns),
        slurm_provider=Slurm(
            user_name=user_name, extra_args=squeue_args, script_path=script_path
        ),
        configs=configs,
    )
    ...
    for compute_type in configs["compute_types"]:
        ...
        if old_workers < 0:
            pool.run_cleanup(
                slurm_running_df, abs(old_workers), compute_type, cluster=compute_type
            )
        # if new_workers > 0:
