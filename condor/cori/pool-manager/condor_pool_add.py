...
def get_current_slurm_workers():
    # returns csv for with SLURM_ID,PARTITION,NAME,STATE,START-TIME,END-TIME
    # Checked on
    squeue_cmd = f"""squeue --clusters=all --format="%.18i %.9P %.40j %.8u %.10T %S %e" -u {accnt_name} | grep {accnt_name} | awk '{{print $1","$2","$3","$5","$6","$7}}' """
    so, se, ec = run_sh_command(squeue_cmd, show_stdout=False)
    if ec != 0:
...
def get_needed_condor_workers() -> Dict:
    ...
    return {'normal': normal_job, 'exvivo': exvivo_job, 'exvivo_shared': shared_job}
