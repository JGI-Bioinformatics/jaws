workflow jgi_rqcfilter {
    ...
    scatter(file in input_files) {
        call rqcfilter{
             input:  input_file=file, container=bbtools_container
        }
    }
...
task rqcfilter {
     ...
     command {
        ...
        set -eo pipefail
        rqcfilter2.sh -Xmx105g threads=${dollar}(grep "model name" /proc/cpuinfo | wc -l) jni=t in=${input_file} path=filtered rna=f trimfragadapter=t qtrim=r trimq=0 maxns=3 maq=3 minlen=51 mlf=0.33 phix=t removehuman=t removedog=t removecat=t removemouse=t khist=t removemicrobes=t sketch kapa=t clumpify=t tmpdir= barcodefilter=f trimpolyg=5 usejni=f rqcfilterdata=/refdata/nmdc/RQCFilterData  > >(tee -a ${filename_outlog}) 2> >(tee -a ${filename_errlog} >&2)
     }
     output {
...
task make_output{
    ...
    command{
        for i in ${sep=' ' rqcfilter_output}; do
