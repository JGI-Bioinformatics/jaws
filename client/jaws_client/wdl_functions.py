        ...
        if d:
            ...
            if os.path.isdir(previous_dir):
                ...
                if not bool(st.st_mode & stat.S_IXOTH):
                    print(
                        "     Permissions failed for the following directory: {}".format(
                            previous_dir
                        )
                    )
                    return 0
            if os.path.isfile(previous_dir):
                ...
                if not bool(st.st_mode & stat.S_IROTH):
                    print(
                        "     Permissions failed for the following file: {}".format(
                            previous_dir
                        )
                    )
                    return 0
...
def searchCommandShifter(wdl):
    ...
    if search_for_shifter:
        print(
            "Error: You should not call shifter in the command stanza. "
    ...
    if search_for_docker:
        print(
            "Error: You should not call docker in the command stanza. "
...
def checkRuntimeFormat(wdl):
    ...
    runtime_matches = re.findall(
        r"runtime\s*{(.*?)(?:}\s*}|output|command)", text, re.DOTALL
    )
    for match in runtime_matches:
        ...
        if not found_something:
            print(
                "Warning: There was a runtime stanza that was empty. Will continue anyway."
            )
            continue
        ...
        if not get_keys:
            print(
                "There doesn't seem to be any valid variables specified in the runtime stanza. "
        ...
        for key in get_keys:
            if key not in accepted_keys:
                print(
                    "error: {} is not an excepted key. please make sure "
                    "you use only the following keys in the runtime stanza:\n {}".format(
                        key, ", ".join(accepted_keys)
                    )
                )
                return 0
