"""
JAWS User class provides authentication vs Globus and JAWS.  And provides the auth header method used by all requests vs JAWS Central.
"""
...
DEBUG = False
if "JAWS_DEBUG" in os.environ and os.environ["JAWS_DEBUG"]: DEBUG = True
...
class User:
    ...
    def __init__(self):
        ...
        if DEBUG: print("Loading %s" % (self.config_file,))
...
#            self.login()
    ...
    def login(self):
        ...
        if DEBUG: print("Authenticating againt Globus")
        try:
        ...
        self.config["TRANSFER"]["expires_at_seconds"] = str(transfer["expires_at_seconds"])
        ...
        if DEBUG: print("Saving token to file")
    ...
    def valid_token(self):
        ...
        """
        if DEBUG: print("Checking token with %s" % (self.header(),))
        url = "%s/user" % (JAWS_URL,)
        try:
        ...
        if DEBUG: print("Welcome, back (valid token)")
#        result = r.json()
...
#            self.config.write_new_config_file()
    ...
    def auth_client(self):
        ...
        access_token = self.config["AUTH"]["access_token"]
        if DEBUG: print("User has auth token: %s" % (access_token,))
